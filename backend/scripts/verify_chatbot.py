...
def _install_minimal_stubs():
    ...
    # installed its own, more minimal app.platform.config stub lacking the
    ...
    fake_config = types.ModuleType("app.platform.config")
    class FakeSettings:
        GEMINI_ENABLED = True
    ...
    if "app.modules.ai.contracts" not in sys.modules:
        fake_chat_schema = types.ModuleType("app.modules.ai.contracts")
        class ProposedAction:
            def __init__(self, **kw):
                self.__dict__.update(kw)
        fake_chat_schema.ProposedAction = ProposedAction
        fake_chat_schema.ChatContext = object
        fake_chat_schema.sanitize_untrusted_text = lambda x: x
        sys.modules["app.modules.ai.contracts"] = fake_chat_schema

    if "app.modules.communications.services" not in sys.modules:
        fake_notif = types.ModuleType("app.modules.communications.services")
        fake_notif.NotificationService = object
        sys.modules["app.modules.communications.services"] = fake_notif

    if "app.modules.auth.auth" not in sys.modules:
        fake_user_model = types.ModuleType("app.modules.auth.auth")
        fake_user_model.User = object
        sys.modules["app.modules.auth.auth"] = fake_user_model
