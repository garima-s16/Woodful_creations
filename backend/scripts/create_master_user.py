...
from app import models  # noqa: F401 - ensures every model (including Employee, the
# target of User.employee_id's ForeignKey) is registered on the shared Base before
# the User mapper is configured. Without this, creating a User in a fresh process
# that has never imported anything else fails with "Foreign key associated with
# column 'users.employee_id' could not find table 'employees'" - a real, observed
# failure fixed by this import, not a schema or migration problem.
