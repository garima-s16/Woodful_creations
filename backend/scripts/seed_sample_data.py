...
from app.platform.database.database import SessionLocal, engine
...
from app.platform.database.id_generator import generate_business_id, generate_unique_code
...
def seed_supplier_materials(db, suppliers, materials):
    ...
    for row in _workbook_rows("Supplier Material Pricing"):
        ...
        db.add(SupplierMaterial(
            ...
            supplier_price=Decimal(str(row.get("Price"))),
...
def seed_product_materials(db, products, materials):
    ...
    for row in _workbook_rows("BOM Examples"):
        ...
        db.add(ProductMaterial(product_id=product.id, material_id=material.id,
                                quantity_required=Decimal(str(row.get("Quantity"))), unit=row.get("Unit")))
...
def seed_purchases(db, suppliers, materials):
    ...
    for row in _workbook_rows("Purchases"):
        ...
        db.add(Purchase(
            ...
            quantity=qty, unit=row.get("Unit"), rate=Decimal(str(row.get("Rate"))),
            taxable_value=Decimal(str(row.get("Taxable Value"))),
            gst_percent=Decimal(str(row.get("GST %"))), gst_amount=Decimal(str(row.get("GST Amount"))),
            invoice_total=Decimal(str(row.get("Invoice Total"))), payment_status=row.get("Payment Status"),
            business_id=generate_business_id(db),
        ))
...
def seed_salary_slips(db, employees):
    ...
    for row in _workbook_rows("Salary Slips"):
        ...
        if (employee.id, month, year) in existing_keys:
            continue
        basic, da, hra, ot = row.get("Basic", 0), row.get("DA", 0), row.get("HRA", 0), row.get("Overtime", 0)
        pf, tds, other = row.get("PF", 0), row.get("TDS", 0), row.get("Other Deductions", 0)
        net_salary = Decimal(str(basic + da + hra + ot - pf - tds - other))
        slip = SalarySlip(
            employee_id=employee.id, month=month, year=year,
            working_days=Decimal(str(row.get("Working Days"))), paid_days=Decimal(str(row.get("Paid Days"))),
            ...
            net_salary=net_salary, status=row.get("Status"),
            business_id=generate_business_id(db),
        )
        db.add(slip)
...
def seed_leaves(db, employees):
    ...
    for row in _workbook_rows("Leaves"):
        ...
        db.add(Leave(
            ...
            days=Decimal(str(row.get("Days"))), reason=row.get("Reason"),
...
def seed_attendance(db, employees):
    ...
    for row in _workbook_rows("Attendance"):
        ...
        db.add(Attendance(date=_d(adate), employee_id=employee.id,
                           ...
                           standard_hours=Decimal("8"), attendance_status=row.get("Status"), remarks=row.get("Remarks"),
                           business_id=generate_business_id(db)))
...
if __name__ == "__main__":
    ...
    try:
        ...
        seed_supplier_materials(db, suppliers, materials)
        ...
        seed_notifications(db, materials)
        ...
        print("=" * 60)
        print(
            "\nEstimates/Orders/Payments/Project Expenses/Issues/Daily Tasks/"
            "Production Jobs are no longer seeded by this script - populate them "
