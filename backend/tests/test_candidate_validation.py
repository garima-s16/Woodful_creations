...
def test_valid_10_digit_phone_accepted(client, test_user):
...
def test_phone_with_wrong_length_rejected(client, test_user):
    _login(client, test_user)
    resp = client.post("/api/candidates/", json={"name": "Short Phone Candidate", "phone": "98765"})
    assert resp.status_code == 422


def test_phone_with_non_digits_rejected(client, test_user):
    _login(client, test_user)
    resp = client.post("/api/candidates/", json={"name": "Letters Phone Candidate", "phone": "98765abcde"})
    assert resp.status_code == 422
