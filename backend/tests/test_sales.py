"""Sales domain tests: estimate/order validation and status,
health/risk/authorization, basics/line-items, profitability,
balance/exports, versioning/risk, payments, and imports. Combines
all former test_*.py files under tests/modules/sales/."""
import io
import re
...
# --- test_estimate_order_validation_and_status.py ---
...
def _create_client_estimate(client, name="Estimate Test Client", phone="9812300001"):
...
def test_estimate_business_id_is_10_char_alphanumeric(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client)
...
def test_estimate_business_ids_increment(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300002")
...
def test_estimate_code_never_client_supplied(client, test_user):
    ...
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300003")
...
def test_estimate_line_item_totals_computed_server_side(client, test_user):
    ...
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300004")
...
def test_estimate_line_item_zero_quantity_rejected(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300025")
    resp = client.post("/api/estimates/", json={
        "client_id": c["id"],
...
def test_estimate_line_item_negative_rate_rejected(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300026")
    resp = client.post("/api/estimates/", json={
        "client_id": c["id"],
        "line_items": [{"description": "Item", "category": "Material", "quantity": "1", "unit": "Nos", "rate": "-500"}],
    })
...
def test_estimate_negative_discount_rejected(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300027")
...
def test_estimate_tax_percent_over_100_rejected(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300028")
...
def test_order_item_negative_quantity_rejected(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300029")
    resp = client.post("/api/orders/", json={
...
def test_order_item_negative_rate_rejected(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300030")
    resp = client.post("/api/orders/", json={
        "client_id": c["id"], "order_date": "2026-07-01T00:00:00",
...
def test_order_item_unknown_product_id_rejected(client, test_user):
    ...
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300031")
    resp = client.post("/api/orders/", json={
        "client_id": c["id"], "order_date": "2026-07-01T00:00:00",
...
def test_direct_order_with_two_real_products(client, test_user):
    ...
    c = _create_client_estimate(client, phone="9812300032")
...
def test_arbitrary_estimate_status_rejected(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300005")
    est = _create_estimate(client, c["id"]).json()
...
def test_draft_to_sent_allowed(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300006")
    est = _create_estimate(client, c["id"]).json()
...
def test_draft_can_go_directly_to_approved(client, test_user):
    ...
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300007")
    est = _create_estimate(client, c["id"]).json()
...
def test_rejected_is_terminal(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300008")
    est = _create_estimate(client, c["id"]).json()
...
def test_approved_can_be_cancelled(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300009")
    est = _create_estimate(client, c["id"]).json()
    client.put(f"/api/estimates/{est['id']}", json={"status": "sent"})
...
def test_finalized_estimate_status_only_change_still_allowed(client, test_user):
    ...
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300012")
    est = _create_estimate(client, c["id"]).json()
    client.put(f"/api/estimates/{est['id']}", json={"status": "sent"})
...
def test_finalized_estimate_line_items_edit_rejected(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300013")
    est = _create_estimate(client, c["id"]).json()
    client.put(f"/api/estimates/{est['id']}", json={"status": "sent"})
    client.put(f"/api/estimates/{est['id']}", json={"status": "rejected"})
...
def test_draft_estimate_content_edit_allowed(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300014")
    est = _create_estimate(client, c["id"]).json()
...
def test_conversion_requires_approved_status(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300015")
...
def test_conversion_of_rejected_estimate_rejected(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300016")
    est = _create_estimate(client, c["id"]).json()
    client.put(f"/api/estimates/{est['id']}", json={"status": "sent"})
    client.put(f"/api/estimates/{est['id']}", json={"status": "rejected"})
    resp = client.post("/api/orders/", json={
...
def test_approved_estimate_converts_successfully(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300017")
    est = _create_estimate(client, c["id"]).json()
...
def test_estimate_manual_status_cannot_be_set_to_closed(client, test_user):
    ...
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300033")
    est = _create_estimate(client, c["id"]).json()
    client.put(f"/api/estimates/{est['id']}", json={"status": "sent"})
    approved = client.put(f"/api/estimates/{est['id']}", json={"status": "approved"}).json()
...
def test_closed_estimate_cannot_be_converted_again(client, test_user):
    ...
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300034")
    est = _create_estimate(client, c["id"]).json()
...
def test_duplicate_conversion_prevented(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300018")
    est = _create_estimate(client, c["id"]).json()
    _approve(client, est["id"])
    first = client.post("/api/orders/", json={
        "from_estimate_id": est["id"], "client_id": c["id"], "order_date": "2026-07-01T00:00:00",
...
def test_conversion_preserves_client_relationship(client, test_user):
    _login(client, test_user)
    c = _create_client_estimate(client, phone="9812300019")
    est = _create_estimate(client, c["id"]).json()
    _approve(client, est["id"])
...
# --- test_order_health_risk_and_authorization.py ---
...
# --- test_orders_basics_and_line_items.py ---
...
def _create_order_basics(client, client_id):
    resp = client.post("/api/orders/", json={
...
# --- test_estimate_items_and_order_profitability.py ---
...
def test_order_with_no_material_issued_has_zero_material_cost(client, test_user):
    ...
    expense_resp = client.post("/api/project-expenses/", json={
        "order_id": order["id"], "date": "2026-08-15T00:00:00", "category": "Labour", "amount": "5000.00",
    })
    assert expense_resp.status_code == 201, expense_resp.text
...
# --- test_order_balance_and_exports.py ---
...
def test_order_estimate_pdf_downloads(client, test_user):
    ...
    # Distinct from an actual Estimate's own quote.pdf filename
    # (estimate-{estimate_code}.pdf) - both are estimate-STYLED
    # documents but for different records, and sharing one filename
    # pattern made them hard to tell apart once downloaded.
    assert f'filename="order-estimate-{order["order_code"]}.pdf"' in resp.headers["content-disposition"]
...
# --- test_estimate_versioning_and_order_risk.py ---
...
def test_invoice_email_failure_surfaces_specific_reason(client, test_user, monkeypatch):
    """Production-readiness fix, issue 2 - the API response must show
    the real, categorized reason (e.g. "login was rejected"), not the
    generic "misconfigured" message regardless of actual cause."""
    from app.modules.communications.services import EmailService

    def fake_send_email(self, *args, **kwargs):
    ...
    resp = client.post(f"/api/orders/{order['id']}/send-email", params={"kind": "invoice"}, json={
        "recipient_email": "client@example.com", "subject": "Invoice", "body": "Please find attached.",
    })
    assert resp.status_code == 502
    detail = resp.json()["detail"]
    assert "login was rejected" in detail
    assert "misconfigured" not in detail  # the old, uninformative generic wording must be gone for this specific case
...
def _create_client_payment(client, name="Payment Test Client"):
    resp = client.post("/api/clients/", json={"name": name, "phone": "9999999998"})
    assert resp.status_code == 201
    return resp.json()["id"]


def _create_order_payment(client, client_id, order_value="100000.00"):
    resp = client.post("/api/orders/", json={
        "client_id": client_id, "project_type": "Wardrobe",
...
def test_cash_payment_gets_auto_generated_reference(client, test_user):
    _login(client, test_user)
    client_id = _create_client_payment(client)
    order = _create_order_payment(client, client_id)
...
def _create_estimate_test_client_and_product(client):
...
def test_estimate_import_commit_creates_real_persisted_estimate(client, test_user):
    _login(client, test_user)
    client_id, product_id = _create_estimate_test_client_and_product(client)
...
def _create_order_test_client_and_product(client):
    client_id = client.post("/api/clients/", json={
...
def test_order_import_commit_creates_real_persisted_order(client, test_user):
    _login(client, test_user)
    client_id, product_id = _create_order_test_client_and_product(client)
...
def test_order_import_commit_rejects_invalid_product_id(client, test_user):
    _login(client, test_user)
    client_id, _ = _create_order_test_client_and_product(client)
...
def test_dispatch_check_blocks_when_balance_outstanding(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300101")["id"]
    order = _create_order_basics(client, client_id)
    # advance (20000) < order_value (100000), so a real balance remains.
    resp = client.get(f"/api/orders/{order['id']}/dispatch-check")
    assert resp.status_code == 200
    body = resp.json()
    assert body["ready_to_dispatch"] is False
    assert body["outstanding_balance"] == 80000.0
    assert body["reason"] is not None and "80000" in body["reason"]


def test_dispatch_check_allows_when_balance_zero(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300102")["id"]
    order = client.post("/api/orders/", json={
        "client_id": client_id, "order_date": "2026-08-01T00:00:00",
        "order_value": "50000.00", "advance": "50000.00",
    }).json()
    resp = client.get(f"/api/orders/{order['id']}/dispatch-check")
    assert resp.status_code == 200
    body = resp.json()
...
def test_dispatch_check_requires_master(client, test_user, db_session):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300103")["id"]
    order = _create_order_basics(client, client_id)
    client.post("/api/auth/logout")
    user = User(username="dispatchrbacuser", email="dispatchrbacuser@example.com", full_name="Dispatch RBAC User",
                password_hash=hash_password("UserPass1!"), role="user", is_active=True)
    db_session.add(user)
    db_session.commit()
    client.post("/api/auth/login", json={"identifier": "dispatchrbacuser@example.com", "password": "UserPass1!"})
    resp = client.get(f"/api/orders/{order['id']}/dispatch-check")
    assert resp.status_code == 403


def test_dispatch_check_order_not_found(client, test_user):
    _login(client, test_user)
    resp = client.get("/api/orders/999999999/dispatch-check")
    assert resp.status_code == 404


def test_update_order_blocks_completion_with_outstanding_balance(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300104")["id"]
    order = _create_order_basics(client, client_id)
    resp = client.put(f"/api/orders/{order['id']}", json={"delivery_status": "Completed"})
    assert resp.status_code == 400
    assert "outstanding balance" in resp.json()["detail"].lower()
    # Never silently applied - re-fetch and confirm delivery_status was NOT changed.
    refetched = client.get(f"/api/orders/{order['id']}").json()
    assert refetched["delivery_status"] != "Completed"


def test_update_order_override_without_reason_rejected(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300105")["id"]
    order = _create_order_basics(client, client_id)
    resp = client.put(f"/api/orders/{order['id']}", json={
        "delivery_status": "Completed", "override_balance_guardrail": True,
    })
    assert resp.status_code == 400
    assert "override_reason" in resp.json()["detail"]


def test_update_order_override_with_reason_succeeds_and_is_audited(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300106")["id"]
    order = _create_order_basics(client, client_id)
    resp = client.put(f"/api/orders/{order['id']}", json={
        "delivery_status": "Completed", "override_balance_guardrail": True,
        "override_reason": "Client picked up in person, balance to follow next week.",
    })
    assert resp.status_code == 200
    assert resp.json()["delivery_status"] == "Completed"

    audit = client.get("/api/audit-logs/", params={"module_name": "orders", "action": "dispatch_with_outstanding_balance_override"})
    assert audit.status_code == 200
    matching = [a for a in audit.json() if a["record_id"] == order["id"]]
    assert len(matching) == 1
    assert matching[0]["new_value"]["override_reason"] == "Client picked up in person, balance to follow next week."


def test_update_order_completion_allowed_when_balance_already_zero(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300107")["id"]
    order = client.post("/api/orders/", json={
        "client_id": client_id, "order_date": "2026-08-01T00:00:00",
        "order_value": "30000.00", "advance": "30000.00",
    }).json()
    resp = client.put(f"/api/orders/{order['id']}", json={"delivery_status": "Completed"})
    assert resp.status_code == 200
    assert resp.json()["delivery_status"] == "Completed"
...
def test_update_order_already_completed_not_reblocked(client, test_user):
    """Re-saving an already-Completed order (no real Pending->Completed
    transition happening) must not re-trigger the guardrail, even with
    an outstanding balance - the guardrail is about the transition, not
    a standing property of the order."""
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300108")["id"]
    order = client.post("/api/orders/", json={
        "client_id": client_id, "order_date": "2026-08-01T00:00:00",
        "order_value": "30000.00", "advance": "30000.00",
    }).json()
    first = client.put(f"/api/orders/{order['id']}", json={"delivery_status": "Completed"})
    assert first.status_code == 200
    # A later payment reversal / adjustment could reopen a balance on an
    # already-completed order in principle; re-saving Completed again
    # must not require an override merely because it's already terminal.
    resp = client.put(f"/api/orders/{order['id']}", json={"delivery_status": "Completed", "remarks": "no-op resave"})
    assert resp.status_code == 200


def test_update_order_non_completion_status_change_not_blocked(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300109")["id"]
    order = _create_order_basics(client, client_id)
    resp = client.put(f"/api/orders/{order['id']}", json={"delivery_status": "In Progress"})
    assert resp.status_code == 200
...
def test_create_approved_specification_is_version_one(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300201")["id"]
    order = _create_order_basics(client, client_id)
    resp = client.post(f"/api/orders/{order['id']}/approved-specifications", json={
        "material": "MDF 18mm", "finish": "Laminate", "colour": "Walnut",
        "approved_by": "Mr. Sharma",
    })
    assert resp.status_code == 201
    body = resp.json()
    ...
    assert body["material"] == "MDF 18mm"


def test_create_approved_specification_supersedes_previous_version(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300202")["id"]
    order = _create_order_basics(client, client_id)
    first = client.post(f"/api/orders/{order['id']}/approved-specifications", json={
        "material": "MDF 18mm", "colour": "Walnut", "approved_by": "Mr. Sharma",
    }).json()

    second = client.post(f"/api/orders/{order['id']}/approved-specifications", json={
        "material": "MDF 18mm", "colour": "Wenge", "approved_by": "Mr. Sharma",
        "notes": "Client changed mind on colour after seeing the sample.",
    })
    assert second.status_code == 201
    second_body = second.json()
    assert second_body["version"] == 2
    assert second_body["supersedes_id"] == first["id"]
    assert second_body["status"] == "approved"

    # The first version must now read as superseded, never silently
    # deleted or overwritten - full history stays reconstructable.
    history = client.get(f"/api/orders/{order['id']}/approved-specifications").json()
    by_id = {s["id"]: s for s in history}
    assert by_id[first["id"]]["status"] == "superseded"
    assert by_id[second_body["id"]]["status"] == "approved"


def test_list_approved_specifications_most_recent_first(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300203")["id"]
    order = _create_order_basics(client, client_id)
    client.post(f"/api/orders/{order['id']}/approved-specifications", json={"material": "MDF", "approved_by": "A"})
    client.post(f"/api/orders/{order['id']}/approved-specifications", json={"material": "Ply", "approved_by": "A"})
    client.post(f"/api/orders/{order['id']}/approved-specifications", json={"material": "MR Ply", "approved_by": "A"})
...
def test_approved_specification_requires_master(client, test_user, db_session):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300204")["id"]
    order = _create_order_basics(client, client_id)
    client.post("/api/auth/logout")
    user = User(username="specrbacuser", email="specrbacuser@example.com", full_name="Spec RBAC User",
                password_hash=hash_password("UserPass1!"), role="user", is_active=True)
    db_session.add(user)
    db_session.commit()
    client.post("/api/auth/login", json={"identifier": "specrbacuser@example.com", "password": "UserPass1!"})
    resp = client.post(f"/api/orders/{order['id']}/approved-specifications", json={"material": "MDF", "approved_by": "A"})
    assert resp.status_code == 403


def test_approved_specification_sample_photo_path_too_long_rejected(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300205")["id"]
    order = _create_order_basics(client, client_id)
    resp = client.post(f"/api/orders/{order['id']}/approved-specifications", json={
        "material": "MDF", "approved_by": "A", "sample_photo_path": "x" * 2001,
    })
    assert resp.status_code == 400


def test_approved_specification_order_not_found(client, test_user):
    _login(client, test_user)
    resp = client.post("/api/orders/999999999/approved-specifications", json={"material": "MDF", "approved_by": "A"})
    assert resp.status_code == 404
...
def test_cost_drift_reports_no_drift_when_cost_unchanged(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300301")["id"]
    product = client.post("/api/products/", json={
        "name": "Drift Test Product A", "unit": "Nos", "cost_price": "5000",
    }).json()
    estimate = client.post("/api/estimates/", json={
        "client_id": client_id,
        "line_items": [{
            "description": "Cabinet", "category": "Material", "quantity": "2",
            "unit": "Nos", "rate": "8000", "product_id": product["id"],
        }],
    }).json()

    resp = client.get(f"/api/estimates/{estimate['id']}/cost-drift")
    assert resp.status_code == 200
    body = resp.json()
    assert body["checked_line_items"] == 1
    assert body["drifted_line_items"] == 0
    assert body["skipped_line_items"] == 0
    assert float(body["cost_delta_total"]) == 0.0


def test_cost_drift_detects_increased_product_cost(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300302")["id"]
    product = client.post("/api/products/", json={
        "name": "Drift Test Product B", "unit": "Nos", "cost_price": "5000",
    }).json()
    estimate = client.post("/api/estimates/", json={
        "client_id": client_id,
        "line_items": [{
    ...
    # Cost rises after the estimate was quoted - the exact scenario this
    # feature exists to surface before confirmation.
    update = client.put(f"/api/products/{product['id']}", json={"cost_price": "6500"})
    assert update.status_code == 200

    resp = client.get(f"/api/estimates/{estimate['id']}/cost-drift")
    assert resp.status_code == 200
    body = resp.json()
    assert body["drifted_line_items"] == 1
    assert float(body["original_cost_total"]) == 10000.0  # 5000 * 2
    assert float(body["current_cost_total"]) == 13000.0   # 6500 * 2
    assert float(body["cost_delta_total"]) == 3000.0
    assert body["current_margin_percent"] is not None
    assert float(body["current_margin_percent"]) < float(body["original_margin_percent"])
    item = body["items"][0]
    assert float(item["cost_delta"]) == 3000.0


def test_cost_drift_skips_custom_line_item_without_product(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300303")["id"]
    estimate = client.post("/api/estimates/", json={
        "client_id": client_id,
        "line_items": [{
            "description": "Custom on-site work", "category": "Labor", "quantity": "1",
            "unit": "Nos", "rate": "5000", "is_custom_item": True,
        }],
    })
    assert estimate.status_code == 201
    estimate_body = estimate.json()

    resp = client.get(f"/api/estimates/{estimate_body['id']}/cost-drift")
    assert resp.status_code == 200
    body = resp.json()
    assert body["checked_line_items"] == 0
    assert body["skipped_line_items"] == 1
    assert body["drifted_line_items"] == 0


def test_cost_drift_requires_master(client, test_user, db_session):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300304")["id"]
    estimate = _create_estimate(client, client_id).json()
    client.post("/api/auth/logout")
    user = User(username="driftrbacuser", email="driftrbacuser@example.com", full_name="Drift RBAC User",
                password_hash=hash_password("UserPass1!"), role="user", is_active=True)
    db_session.add(user)
    db_session.commit()
    client.post("/api/auth/login", json={"identifier": "driftrbacuser@example.com", "password": "UserPass1!"})
    resp = client.get(f"/api/estimates/{estimate['id']}/cost-drift")
    assert resp.status_code == 403


def test_cost_drift_estimate_not_found(client, test_user):
    _login(client, test_user)
    resp = client.get("/api/estimates/999999999/cost-drift")
    assert resp.status_code == 404
...
def test_margin_optimization_finds_cheaper_substitute(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300401")["id"]
    current_product = client.post("/api/products/", json={
        "name": "Margin Test Current Product", "unit": "Nos",
        "category": "Panels", "cost_price": "6000", "selling_price": "10000",
    }).json()
    cheaper_product = client.post("/api/products/", json={
        "name": "Margin Test Cheaper Product", "unit": "Nos",
        "category": "Panels", "cost_price": "5000", "selling_price": "7000",
    }).json()
    estimate = client.post("/api/estimates/", json={
        "client_id": client_id,
        "line_items": [{
            "description": "Panel unit", "category": "Material", "quantity": "2",
            "unit": "Nos", "rate": "10000", "product_id": current_product["id"],
        }],
    }).json()

    resp = client.post(f"/api/estimates/{estimate['id']}/margin-optimization", json={"target_price": "15000"})
    assert resp.status_code == 200
    body = resp.json()
    assert len(body["suggestions"]) == 1
    suggestion = body["suggestions"][0]
    assert suggestion["suggested_product_id"] == cheaper_product["id"]
    assert float(suggestion["suggested_rate"]) == 7000.0
    assert float(suggestion["saving_per_unit"]) == 3000.0
    assert float(suggestion["total_saving"]) == 6000.0  # 3000 * quantity(2)
    assert float(body["current_total"]) == 20000.0  # 10000 * 2
    assert float(body["best_achievable_price"]) == 14000.0  # 20000 - 6000
    assert body["target_reachable"] is True


def test_margin_optimization_target_unreachable(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300402")["id"]
    product = client.post("/api/products/", json={
        "name": "Margin Test Product Unreachable", "unit": "Nos",
        "category": "Panels Unreachable", "cost_price": "6000", "selling_price": "10000",
    }).json()
    estimate = client.post("/api/estimates/", json={
        "client_id": client_id,
        "line_items": [{
            "description": "Panel unit", "category": "Material", "quantity": "1",
            "unit": "Nos", "rate": "10000", "product_id": product["id"],
        }],
    }).json()
    # No cheaper alternative exists in this category, so no suggestions
    # and best_achievable_price stays at current_total.
    resp = client.post(f"/api/estimates/{estimate['id']}/margin-optimization", json={"target_price": "1000"})
    assert resp.status_code == 200
    body = resp.json()
    assert body["suggestions"] == []
    assert float(body["best_achievable_price"]) == float(body["current_total"])
    assert body["target_reachable"] is False


def test_margin_optimization_requires_target_or_min_margin(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300403")["id"]
    estimate = _create_estimate(client, client_id).json()
    resp = client.post(f"/api/estimates/{estimate['id']}/margin-optimization", json={})
    assert resp.status_code == 422


def test_margin_optimization_requires_master(client, test_user, db_session):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300404")["id"]
    estimate = _create_estimate(client, client_id).json()
    client.post("/api/auth/logout")
    user = User(username="marginrbacuser", email="marginrbacuser@example.com", full_name="Margin RBAC User",
                password_hash=hash_password("UserPass1!"), role="user", is_active=True)
    db_session.add(user)
    db_session.commit()
    client.post("/api/auth/login", json={"identifier": "marginrbacuser@example.com", "password": "UserPass1!"})
    resp = client.post(f"/api/estimates/{estimate['id']}/margin-optimization", json={"target_price": "1000"})
    assert resp.status_code == 403


def test_margin_optimization_estimate_not_found(client, test_user):
    _login(client, test_user)
    resp = client.post("/api/estimates/999999999/margin-optimization", json={"target_price": "1000"})
    assert resp.status_code == 404


def test_margin_optimization_never_modifies_estimate(client, test_user):
    """Read-only per the feature's own contract - suggestions are
    informational only, applying one requires the human to edit the
    estimate through the normal update-estimate endpoint."""
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812300405")["id"]
    current_product = client.post("/api/products/", json={
        "name": "Margin Test No-Mutate Product", "unit": "Nos",
        "category": "Panels No Mutate", "cost_price": "6000", "selling_price": "10000",
    }).json()
    client.post("/api/products/", json={
        "name": "Margin Test No-Mutate Cheaper", "unit": "Nos",
        "category": "Panels No Mutate", "cost_price": "5000", "selling_price": "7000",
    })
    estimate = client.post("/api/estimates/", json={
        "client_id": client_id,
        "line_items": [{
    ...
    client.post(f"/api/estimates/{estimate['id']}/margin-optimization", json={"target_price": "1000"})
    refetched = client.get(f"/api/estimates/{estimate['id']}").json()
    assert float(refetched["line_items"][0]["rate"]) == 10000.0
...
def test_build_timeline_order_not_found(client, test_user):
    _login(client, test_user)
    resp = client.get("/api/orders/999999999/build-timeline")
    assert resp.status_code == 404


def test_build_timeline_no_milestones_returns_empty_list(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812340001")["id"]
    order = _create_order_basics(client, client_id)
    resp = client.get(f"/api/orders/{order['id']}/build-timeline")
    assert resp.status_code == 200
    body = resp.json()
    assert body["order_id"] == order["id"]
    assert body["milestones"] == []
    assert body["overall_risk_level"] in ("ON_TRACK", "WATCH", "AT_RISK", "CRITICAL")


def test_build_timeline_classifies_completed_delayed_and_current(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812340002")["id"]
    order = _create_order_basics(client, client_id)
    completed = client.post("/api/milestones/", json={
        "order_id": order["id"], "name": "Design Approval", "target_date": "2020-01-01T00:00:00",
    }).json()
    client.put(f"/api/milestones/{completed['id']}", json={"completed_date": "2020-01-02T00:00:00"})
    client.post("/api/milestones/", json={
        "order_id": order["id"], "name": "Long Overdue Delivery", "target_date": "2020-02-01T00:00:00",
    })
    client.post("/api/milestones/", json={
        "order_id": order["id"], "name": "Far Future Milestone", "target_date": "2099-01-01T00:00:00",
    })

    resp = client.get(f"/api/orders/{order['id']}/build-timeline")
    assert resp.status_code == 200
    by_name = {m["name"]: m for m in resp.json()["milestones"]}
    assert by_name["Design Approval"]["status"] == "completed"
    # The next open milestone by target_date (not yet completed, in the
    # past) is genuinely delayed - never silently reported as "planned".
    assert by_name["Long Overdue Delivery"]["status"] == "delayed"
    assert by_name["Far Future Milestone"]["status"] == "planned"


def test_build_timeline_requires_auth(client):
    resp = client.get("/api/orders/1/build-timeline")
    assert resp.status_code == 401
...
def test_delivery_promise_evaluation_past_date_is_not_feasible(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812340003")["id"]
    order = _create_order_basics(client, client_id)
    resp = client.get(f"/api/orders/{order['id']}/delivery-promise", params={"requested_date": "2020-01-01T00:00:00"})
    assert resp.status_code == 200
    body = resp.json()
    assert body["feasibility"] == "NOT_FEASIBLE"
    assert body["label"] == "PREDICTION"


def test_delivery_promise_evaluation_clean_order_far_future_is_feasible(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812340004")["id"]
    order = _create_order_basics(client, client_id)
    resp = client.get(f"/api/orders/{order['id']}/delivery-promise", params={"requested_date": "2099-06-01T00:00:00"})
    assert resp.status_code == 200
    body = resp.json()
    # An order with no products/BOM and no production jobs at all has
    # nothing to be short of or blocked by - a clean, far-future date
    # must be reported feasible, never a fabricated risk.
    assert body["feasibility"] == "FEASIBLE"
    assert body["bottlenecks"] == []


def test_delivery_promise_evaluation_order_not_found(client, test_user):
    _login(client, test_user)
    resp = client.get("/api/orders/999999999/delivery-promise", params={"requested_date": "2099-01-01T00:00:00"})
    assert resp.status_code == 404


def test_delivery_promise_record_requires_master(client, test_user, db_session):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812340005")["id"]
    order = _create_order_basics(client, client_id)
    client.post("/api/auth/logout")
    user = User(username="deliverypromiseuser", email="deliverypromiseuser@example.com", full_name="Delivery Promise User",
                password_hash=hash_password("UserPass1!"), role="user", is_active=True)
    db_session.add(user)
    db_session.commit()
    client.post("/api/auth/login", json={"identifier": "deliverypromiseuser@example.com", "password": "UserPass1!"})
    resp = client.post(f"/api/orders/{order['id']}/delivery-promise", json={
        "promised_date": "2099-01-01T00:00:00", "reason": "Should be blocked",
    })
    assert resp.status_code == 403


def test_delivery_promise_record_writes_order_and_audit_trail(client, test_user):
    _login(client, test_user)
    client_id = _create_client_estimate(client, phone="9812340006")["id"]
    order = _create_order_basics(client, client_id)
    resp = client.post(f"/api/orders/{order['id']}/delivery-promise", json={
        "promised_date": "2099-03-15T00:00:00", "reason": "Confirmed with client after capacity review.",
    })
    assert resp.status_code == 200
    assert resp.json()["promised_date"].startswith("2099-03-15")

    refetched = client.get(f"/api/orders/{order['id']}").json()
    assert refetched["delivery_date"].startswith("2099-03-15")

    audit = client.get("/api/audit-logs/", params={"module_name": "orders", "action": "delivery_date_promised"})
    assert audit.status_code == 200
    matching = [a for a in audit.json() if a["record_id"] == order["id"]]
    assert len(matching) == 1
