"""Tests for Family 15 (Business AI Agents). The core architectural
claim under test: every agent query goes through the exact same
authorized pipeline as the chatbot, so every guarantee already proven
for the chatbot (RBAC, real-data grounding, confirmation-required
actions) must hold identically through the agent route too - these
tests prove that inheritance actually holds, not just assume it from
the code structure."""
