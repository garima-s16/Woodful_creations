"""Operations domain schemas - daily employee tasks, production floor
jobs, material issues, order milestones, and project expenses. Consolidated from five
separate modules that all belong to the same operational-execution
feature area."""
...
# The real, existing task status vocabulary (confirmed against
# TaskDetailPage.jsx's actual status dropdown, not invented here).
DAILY_TASK_STATUSES = {"TO DO", "DOING", "DONE", "BLOCKED"}
# The real, existing task priority vocabulary (confirmed against
# TaskDetailPage.jsx's actual priority dropdown, not invented here) -
# previously defined nowhere in the backend, so an arbitrary string
# could be set via direct API call or import despite the UI only ever
# offering these four.
DAILY_TASK_PRIORITIES = {"Low", "Normal", "High", "Urgent"}
...
# The only statuses the application's own UI offers (confirmed against
# ProductionJobsPage.jsx's real status dropdown, not invented here) -
# "Blocked" is not a production job status; blocker_reason is a
# separate, freestanding field that can accompany any status.
PRODUCTION_JOB_STATUSES = {"Not Started", "In Progress", "Completed"}
