...
from app.platform.security.security import create_access_token, verify_password, hash_password, get_current_user
from app.platform.security.rate_limit import (
    rate_limit,
...
from app.modules.auth.schemas import LoginResponse, MobileLoginResponse, UserLogin, ForgotPasswordRequest, ResetPasswordRequest
...
# A fixed, publicly-known bcrypt-format placeholder - never a real
# credential. Used only so a nonexistent-account login still runs a
# genuine password verification, taking comparable time to a real
# wrong-password attempt rather than returning early - otherwise an
# attacker could distinguish "valid username, wrong password" from
# "no such account" purely by response time, even with an identical
# error message.
