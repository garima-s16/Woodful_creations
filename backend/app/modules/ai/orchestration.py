...
from app.modules.sales.services import (
    ...
    _route_order_status_query, _continue_order_status_query,
)
...
class ChatService:
    ...
    def process_message(message: str, db: Session, user_role: str = "user",
        ...
        # Continuing an order-status question across a client/order
        # disambiguation clarification (see _continue_order_status_query) -
        # the fix for a real bug: without this, a clarification reply
        # like "Siddharth" alone lost the original "where is the order"
        # intent entirely and fell through to a generic tool instead.
        if context and context.pending and context.pending.get("type") == "order_status_query":
            result = _continue_order_status_query(m, context.pending, db)
            if result:
                return result

        order_status_result = _route_order_status_query(m, db, user_role, context)
        ...
        text, suggestions, proposed_action, clarification, records = ChatService._normalize_dispatch_result(
            ChatService._dispatch(m, db, user_role, context, current_employee_id)
        )
        ...
        return text, suggestions, proposed_action, clarification, records
    ...
    def _normalize_dispatch_result(
        result: Union[Tuple[str, List[str], List[dict]], Tuple[str, List[str], Optional[object], Optional[dict], List[dict]]],
    ) -> Tuple[str, List[str], Optional[object], Optional[dict], List[dict]]:
        """The one authoritative normalization point for every handler
        _dispatch might return the result of, directly or via
        pass-through (return some_handler(...)). Handlers overwhelmingly
        return the 3-element (text, suggestions, records) shape - no
        proposed action, no clarification - but a few (e.g.
        _route_product_search) already need to propose a data-modifying
        action and return the full 5-element (text, suggestions,
        proposed_action, clarification, records) shape instead. Both are
        valid; callers must go through this function rather than
        unpacking _dispatch's return value directly, which is exactly
        what previously crashed with "ValueError: too many values to
        unpack" the moment a 5-element result reached a 3-element
        unpack. This does not change any handler's return value - it
        only widens whichever shape it receives to the common one."""
        if len(result) == 5:
            return result
        if len(result) == 3:
            text, suggestions, records = result
            return text, suggestions, None, None, records
        raise ValueError(
            f"Chat handler returned {len(result)} values; expected 3 (text, suggestions, "
            f"records) or 5 (text, suggestions, proposed_action, clarification, records)."
        )

    @staticmethod
    def _dispatch(m: str, db: Session, user_role: str, context: Optional[ChatContext],
                  current_employee_id: Optional[int] = None):
        """Returns EITHER a 3-tuple (text, suggestions, records) - the
        shape most handlers below use - OR a 5-tuple (text, suggestions,
        proposed_action, clarification, records) for the handful of
        handlers (e.g. _route_product_search) that need to propose an
        action. Callers must always go through
        ChatService._normalize_dispatch_result(...) rather than
        unpacking this method's return value directly - see that
        ...
        if excel_query:
            return excel_query
        # Sales-history intent must be checked before the generic named-
        # client order lookup below - "Sanket's sales history" would
        # otherwise resolve "Sanket" to a client and answer with just
        # their MOST RECENT order's status (_route_client_order_query),
        # never reaching the sales-history handler that actually
        # answers what was asked. _route_sales_history_query itself
        # returns None immediately for anything that isn't a "sales
        # history"/"history" request, so this reordering changes
        # nothing for ordinary "how is Sanket's order doing" phrasings.
        ...
        if any(w in m for w in ["stock", "material", "inventory"]):
            return _stock_summary(db, user_role)
        # Payment/profitability intent must be checked before the
        # generic "order"/"client" catch-alls below - a message like
        # "which orders have outstanding payments" contains "orders"
        # and would otherwise be swallowed by _orders_summary (a plain
        # company-wide order list, with no payment information at all)
        # before ever reaching the payment-specific handler that
