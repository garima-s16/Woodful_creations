...
def _tool_get_salary_advance_status(db: Session, args: dict, user_role: str) -> Tuple[str, List[dict]]:
    ...
    from app.modules.hr.services import get_salary_advance_summary
...
def _tool_get_overtime_status(db: Session, args: dict, user_role: str) -> Tuple[str, List[dict]]:
    ...
    from datetime import datetime
    from app.modules.hr.services import get_overtime_summary
    now = datetime.utcnow()
...
def _tool_search_documents(db: Session, args: dict, user_role: str) -> Tuple[str, List[dict]]:
    ...
    if user_role not in ("master",):
        from app.modules.documents.api import SENSITIVE_PARENT_TYPES as _SENSITIVE
