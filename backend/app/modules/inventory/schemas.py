...
class MaterialCreate(MaterialBase):
    ...
    # MaterialUpdate and MaterialResponse both already carry this field;
    # MaterialCreate did not, even though api.py's create_material
    # always reads data.attribute_values - an AttributeError on every
...
class MaterialResponse(MaterialBase):
    ...
    # Exposed for transparency/diagnosis - which location (if any) this
    # material's opening_stock counts toward in _location_balance. A
    # material created with none set has opening stock that is
    # correctly counted in current_stock but genuinely un-findable at
    # any specific location until this is filled in (see
    # update_material's own handling of this).
    opening_stock_location_id: Optional[int] = None
...
# Resolves the self-referential forward reference above ("children:
# List[\"LocationTreeResponse\"]") - must run after the class body has
# executed, which is why it's a module-level call right here rather
# than inside the class. This was previously (incorrectly) placed in
# models.py, a different module that neither defines nor imports this
# class - a bare NameError at import time.
...
class DeadStockMatch(BaseModel):
    """One estimate that could potentially use some of an idle
    material's quantity - a possibility to review, never an
    allocation. Nothing here reserves or moves any stock."""
    estimate_id: int
    estimate_code: str
...
class DeadStockItem(BaseModel):
    ...
    potential_matches: List[DeadStockMatch] = []


class DeadStockMatchesResponse(BaseModel):
