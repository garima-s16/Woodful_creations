...
from app.shared.exporters import write_sheet, write_instructions_sheet, add_dropdown_validation
...
from app.modules.clients.schemas import CLIENT_TYPES

TEMPLATE_VERSION = "2.0"  # bumped from 1.0: adds Client Type (mandatory), State, Pincode

# Data-sheet headers use the "*" convention for mandatory fields.
...
def normalize_match_key(name: Optional[str], phone: Optional[str]) -> str:
    ...
    for order intake (see app/modules/clients/matching.py), reused here
...
# Approved Woodful demo data - real Woodful
...
def validate_and_match_row(row: dict, existing_by_key: dict, fuzzy_candidates: Optional[list] = None):
    ...
    endpoint (see app.modules.clients.matching.find_fuzzy_name_matches).
