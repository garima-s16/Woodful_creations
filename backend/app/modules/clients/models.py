...
class Client(BaseModel):
    ...
    # see app/platform/ids.py.generate_business_id. Never client-supplied.
...
CLIENT_ACCESS_TOKEN_PURPOSES = ("estimate_approval", "my_order")
...
class ClientAccessToken(BaseModel):
    ...
    pattern: the raw token is generated with secrets.token_urlsafe and
    given to the client once, only its SHA-256 hash is ever stored, so
    a database read alone can never reconstruct a working link. Unlike
    a password-reset token this is not single-use - a client
    legitimately reopens the same estimate/order link many times - so
    there is no used_at; is_revoked lets staff kill a link explicitly
    (e.g. a superseded estimate), and expires_at is nullable because a
    My Order link should keep working for the life of the order, not
    on a timer.

    subject_type/subject_id point at the Estimate or Order this token
    grants read (and, for estimate_approval, one specific write:
    approve/request-changes) access to - never broader account access,
    and never anything requiring the internal ERP login."""
    __tablename__ = "client_access_tokens"

    token_hash = Column(String(64), nullable=False, unique=True, index=True)
    purpose = Column(String(20), nullable=False)  # one of CLIENT_ACCESS_TOKEN_PURPOSES
    subject_type = Column(String(20), nullable=False)  # "estimate" or "order"
