"""Excel import for Estimates (Family 103 sections 14-16, 20-26) - the
...
are always recomputed server-side via app.utils.calculations.compute_totals,
...
def _resolve_header_row(values, aliases, required_cols):
    resolved = {}
    for idx, raw in enumerate(values):
        if raw is None:
            continue
        # Strip the trailing " *" mandatory-field marker (spec section 3)
        # before alias lookup - the template writes "Client Name *" as a
        # visual cue, but the alias table only knows the bare column name.
...
def _parse_sheet(ws, all_columns, aliases, required_cols, sheet_label) -> List[dict]:
    ...
    for row in ws.iter_rows(min_row=1, max_row=10):
        ...
        resolved = _resolve_header_row(values, aliases, required_cols)
        if resolved:
