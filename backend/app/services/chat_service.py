...
class ChatService:
    ...
    def _handle_low_stock_query(db: Session) -> Tuple[str, List[str]]:
        ...
        if not low_items:
            ...
            for item in low_items:
                label = f"{item.material_type} {item.thickness}mm" if item.thickness else item.material_type
                response += f"- {label}: {item.quantity}/{item.min_quantity} units\n"
    ...
    def _handle_payment_query(db: Session) -> Tuple[str, List[str]]:
        response = "Financial information is available for master users. Use the Payments page for details."
    ...
    def _handle_help_query() -> Tuple[str, List[str]]:
        response = (
            "I can help with: inventory management, stock alerts, estimates, "
            "client information, and more. What do you need?"
        )
    ...
    def _handle_default_query(message: str) -> Tuple[str, List[str]]:
        response = (
            f"I understand you asked about: '{message}'. "
