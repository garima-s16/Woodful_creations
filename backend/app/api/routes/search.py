...
def global_search(q: Optional[str] = Query(None, min_length=1), db: Session = Depends(get_db),
    ...
    employee, task - and returns a single ranked list, each result tagged
    with its record type so the frontend can route to the right detail
    page. Every entity here is already visible to all authenticated
    roles, so this endpoint doesn't need its own extra RBAC beyond that -
    it deliberately excludes role-restricted data (payments, salary,
    candidates, interviews) rather than complicate that per-result."""
    ...
    for c in db.query(Client).filter(
        (Client.name.ilike(like)) | (Client.phone.ilike(like)) | (Client.client_code.ilike(like))
    ...
    for o in db.query(Order).filter(
        (Order.order_code.ilike(like)) | (Order.project_type.ilike(like))
    ).limit(RESULTS_PER_TYPE).all():
    ...
    for e in db.query(Estimate).filter(Estimate.estimate_code.ilike(like)).limit(RESULTS_PER_TYPE).all():
    ...
    for m in db.query(Material).filter(
        (Material.name.ilike(like)) | (Material.material_code.ilike(like))
    ).limit(RESULTS_PER_TYPE).all():
    ...
    for t in db.query(DailyTask).filter(
        (DailyTask.task_description.ilike(like)) | (DailyTask.task_code.ilike(like))
    ).limit(RESULTS_PER_TYPE).all():
