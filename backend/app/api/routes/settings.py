...
async def get_project_statuses(db: Session = Depends(get_db)):
    statuses = db.query(ProjectStatus).all()
    return statuses

@router.post("/project-statuses", response_model=ProjectStatusResponse)
async def create_project_status(status: ProjectStatusCreate, db: Session = Depends(get_db)):
...
async def create_payment_mode(mode: PaymentModeCreate, db: Session = Depends(get_db)):
...
async def create_lead_source(source: LeadSourceCreate, db: Session = Depends(get_db)):
...
async def create_project_type(ptype: ProjectTypeCreate, db: Session = Depends(get_db)):
...
async def create_expense_category(category: ExpenseCategoryCreate, db: Session = Depends(get_db)):
...
async def delete_project_status(status_id: int, db: Session = Depends(get_db)):
