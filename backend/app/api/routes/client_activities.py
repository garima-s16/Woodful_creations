...
from app.schemas.client_activity import ClientActivityCreate, ClientActivityUpdate, ClientActivityResponse
...
def list_pending_follow_ups(
    ...
    round trip per row. Without this endpoint, follow_up_date was
    written on every activity form but never actually surfaced to a
    user anywhere in the app - only the chatbot's ad-hoc suggestion
    list read it."""
