...
│   │   ├── core/                Platform layer: config.py (settings/env), database.py
│   │   │                        (DB session/engine), security.py (password hashing, tokens,
│   │   │                        require_role()), middleware.py, rate_limit.py, audit.py
│   │   │                        (log_action() + AuditLog model/schema), auto_migrate.py,
│   │   │                        constants.py, exceptions.py, storage.py (Drive/local
│   │   │                        storage abstraction)
│   │   │
...
│   │   │   │                     (recompute_totals/profitability), calculations.py,
│   │   │   │                     status_rules.py, quantity_rules.py
...
│   │   │                        modules: pdf_generator.py, exporters.py (Excel/Word),
│   │   │                        document_style.py (PDF/Excel branding), validators.py,
│   │   │                        import_common.py, import_normalize.py
│   │   │
│   │   ├── models/, schemas/, api/routes/, services/, utils/
│   │   │                        What's left after the module split: cross-cutting or
│   │   │                        genuinely platform-level pieces that don't belong to one
│   │   │                        business domain — settings.py, users.py, auth.py,
│   │   │                        id_generator.py (kept here, not shared/, since three
│   │   │                        historical Alembic migrations import it directly),
│   │   │                        helpers.py. Each domain's own reports export lives at
│   │   │                        modules/*/api/reports.py, not here.
│   │   │
