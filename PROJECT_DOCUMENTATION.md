...
The most recent work closed out Families 11-13 (communication, analytics, business automation) and their gap-fix pass: `automation_service.py` gained four rules that didn't exist before (follow-up due, pending estimate response, project deadline approaching, operational summary), and two PDF export routes (`orders/{id}/estimate.pdf`, `estimates/{id}/quote.pdf`) were found and fixed to require `master`, having previously let any authenticated role download financial fields the JSON API already redacts. Family 11 (communication search/insights/draft) and most of Family 12 (analytics/export role gating) were audited and found already correct against the app's real access model — nothing there needed changing; see the automation/communication/analytics paragraphs above for the detail.
...
│   │   │                        locations.py, notifications.py, analytics.py (Family 12 —
│   │   │                        per-domain KPI/trend routes, is_privileged pattern),
│   │   │                        communication.py (Family 11 — search/insights/draft),
│   │   │                        automation.py (Family 13 — rules/logs/run, all master-only),
│   │   │                        reports.py (every Excel/PDF export — check field-level auth
│   │   │                        here matches the on-screen route it mirrors)
│   │   │
...
│   │   │                        analytics "explain" answers gated by user_role),
│   │   │                        notification_service.py (event-driven notification
│   │   │                        generation with dedup + FINANCIAL_NOTIFICATION_TYPES,
│   │   │                        the shared "is this notification money-sensitive" rule),
│   │   │                        automation_service.py (Family 13 — EVENT->CONDITION->ACTION
│   │   │                        rules, one check_* per rule, run_all() orchestrates),
│   │   │                        analytics_service.py (Family 12 — per-domain aggregates,
│   │   │                        is_privileged param nulls financial fields),
│   │   │                        communication_ai_service.py (Family 11 — rule-based
│   │   │                        summarize/extract/draft, explicitly not a real LLM),
│   │   │                        mention_service.py, email_service.py
│   │   │
...
- **An automation rule isn't firing, fires twice, or an existing rule needs a sibling** — `services/automation_service.py`; each rule is one `check_*` static method with its own try/except and `dedup_key`, registered in both `RULES` (the static registry `GET /api/automation/rules` returns) and the `run_all()` tuple — a new rule needs to be added to both, not just written.
- **Analytics/export shows (or hides) financial data it shouldn't for a given role** — check the `is_privileged` flag in `services/analytics_service.py` / `api/routes/analytics.py` first; for exports specifically, compare `api/routes/reports.py` against the on-screen route it mirrors — an export must never expose more than its screen equivalent (see `tests/test_estimate_pdf_export_rbac.py` for the pattern to follow when adding a new export).
- **Communication search/insights/draft looks like it's leaking something** — `api/routes/communication.py`; remember comments/activities are intentionally visible to every role (not a bug), the thing to check is whether a *financial* value (notification content, a redacted field) is leaking through, not whether the record itself is visible.
