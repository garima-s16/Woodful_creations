...
function RateCardsPage() {
  ...
  const load = () => {
    ...
    ratesAPI.list(params).then((res) => setRates(res.data)).finally(() => setPageLoading(false));
  };
  ...
  const fields = [
    ...
    { name: 'location', label: 'Location', placeholder: 'Indore, Madhya Pradesh' },
    { name: 'market_reference_rate', label: 'Market Reference Rate (Indore)', type: 'number', section: 'Pricing' },
    { name: 'woodful_cost_rate', label: 'Woodful Internal Cost', type: 'number', section: 'Pricing' },
    { name: 'woodful_selling_rate', label: 'Woodful Selling Rate', type: 'number', section: 'Pricing' },
    { name: 'overhead_percent', label: 'Overhead %', type: 'number', section: 'Pricing' },
    { name: 'target_margin_percent', label: 'Target Margin %', type: 'number', section: 'Pricing' },
    { name: 'wastage_percent', label: 'Wastage %', type: 'number', section: 'Pricing' },
    { name: 'tax_percent', label: 'Tax %', type: 'number', section: 'Pricing' },
    { name: 'source_type', label: 'Source Type', type: 'select', required: true, section: 'Source',
      options: SOURCE_TYPES.map((s) => ({ value: s, label: s.replace(/_/g, ' ') })) },
    { name: 'confidence', label: 'Confidence', type: 'select', section: 'Source',
      options: CONFIDENCE_LEVELS.map((c) => ({ value: c, label: c.replace('_', ' ') })) },
    { name: 'source_reference', label: 'Source Reference', type: 'textarea', section: 'Source' },
  ...
  return (
    <div className="page">
      ...
      <Table
        columns={columns} data={rates} loading={pageLoading}
