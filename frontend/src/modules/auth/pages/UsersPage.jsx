...
// the login screen never has to wait on a dynamic import - but that
// meant this file's code (a master-only admin page, never rendered
// before authentication) was pulled into the SAME webpack chunk as
// the login code purely because they lived in one file, defeating
// the React.lazy() wrapper App.jsx already had around "UsersPage" -
// that lazy() was importing from a module webpack had already put in
// the main bundle via the static LoginPage import, so it produced no
// actual code-splitting benefit. Moving UsersPage to its own file
// lets App.jsx's existing `React.lazy(() => import('.../UsersPage'))`
// genuinely split this page's code (and the Table/Modal/Form/
// ConfirmDialog machinery it pulls in) out of the login-critical path.
